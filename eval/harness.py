...
def evaluate_investigation(scenario: IncidentScenario, ledger: EvidenceLedger) -> EvaluationResult:
    ...
    cites expected scenario evidence. It intentionally does not claim semantic
    root-cause accuracy; that requires a labelled assertion matcher or manual
    review and should never be faked by string similarity.
    """
    notes: list[str] = []
    ...
    root_cause_evidence_recall = len(expected & supported) / len(expected) if expected else 0.0
    if top is None:
