...
def render_markdown(report: PostmortemReport) -> str:
    ...
    if report.executive_summary:
        lines.extend(render_statement(s) for s in report.executive_summary)
    else:
        lines.append("- No supported summary available.")

    lines.extend(["", "## Timeline"])
    if report.timeline:
        lines.extend(render_statement(s) for s in report.timeline)
    else:
        lines.append("- No timestamped evidence available.")

    lines.extend(["", "## Root cause"])
    lines.append(render_statement(report.root_cause) if report.root_cause else "- No surviving hypothesis available.")

    lines.extend(["", "## Recommended actions"])
    if report.recommended_actions:
        lines.extend(render_statement(s) for s in report.recommended_actions)
    else:
        lines.append("- Escalate to an on-call human for a reviewed action plan.")
...
if __name__ == "__main__":
    ...
    if postmortem.validation_errors:
        print("VALIDATION ERRORS FOUND:", postmortem.validation_errors)
